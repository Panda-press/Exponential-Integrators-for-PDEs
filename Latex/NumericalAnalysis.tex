...
\begin{lemma} \label{lemma:leqm}
    We have that $||e_m^A|| \leq m$ when $A$ is negative semi-definite.
\end{lemma}
\begin{proof}
    \begin{align*}
...
\begin{proof}
    ...
    \end{align*}
    with the bound on $P_hf(t,u_{h,m}^n)$ being the result of Lipschitz continuity by taking $C=P_hf(t,0)$.
\end{proof}
\begin{definition}
